...
import { createInitialUpdateState } from './form-state';
...
export function UserUpdateForm({ userId, defaults }: Props) {
    ...
    <form action={formAction} className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-1 text-sm text-gray-700">
          <span>氏名</span>
          <input defaultValue={defaults.name} name="name" placeholder="山田 太郎" className={fieldClass} />
        </label>

        <label className="flex flex-col gap-1 text-sm text-gray-700">
          <span>メールアドレス</span>
          <input defaultValue={defaults.email} name="email" placeholder="taro@example.com" className={fieldClass} />
        </label>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-1 text-sm text-gray-700">
          <span>役割</span>
          <select defaultValue={defaults.role} name="role" className={fieldClass}>
...
function FormMessage({ state }: { state: UpdateUserFormState }) {
  ...
  return (
    <p className={`text-sm font-medium ${color}`}>
      {state.message}
      {state.status === 'success' && state.refreshedAt ? `（${new Date(state.refreshedAt).toLocaleString()}）` : ''}
    </p>
  );
}
